...
//static const int comp_col = filter_size*DEPTH;
...
__device__ void Mat_ACC(unsigned int C[filter_size][comp_col], int filter_size, int line){
...
//    printf("filter=%d\n", filter_size);
//    printf("total=%d\n", total);
//    printf("line, counter=%d, %u\n", line, counter);
    if (counter > threshold){
        result_device[line] = 1;
    }
    else{
        result_device[line] = 0;
    }
//    result_device[line] = counter;
    __syncthreads();
//    printf("result_ACC=%d\n", result_device[column]);
//    return;
}

__device__ void Mat_XNOR(unsigned int A[filter_size][comp_col], unsigned int B[filter_size][comp_col], unsigned int C[filter_size][comp_col], int line){
    __syncthreads();
...
//    printf("XNOR_CAL time(cycles)=%d\n", time);
    __syncthreads();
//    __syncthreads();
//    start_time = clock();
    Mat_ACC(C, filter_size, line);
//    stop_time = clock();
//    time = (int) (stop_time - start_time);
//    printf("ACC time(cycles)=%d\n", time);
    __syncthreads();
//    printf("result_XNOR=%d\n", result_device[column]);
//    return;
}
...
//    printf("line = %d\n",line);
    ...
    unsigned int (*xA)[comp_col], (*xB)[comp_col], (*xC)[comp_col];
            ...
            for (int d = 0; d<DEPTH; d++) {
                for (int r = 0; r < filter_size; r++) {
                    for (int c = 0; c < filter_size; c++) {
                        pA[r][d*filter_size + c] = i_fmap[fout_c*stride + r][d * input_size + line * stride + c];
...
//            for (int k = 0; k < comp_col; k++) {
//                for (int i = 0; i < filter_size; i++) {
//                    for (int j = 0; j < filter_size * DEPTH; j++) {
//                    }
//                }
//            }
            for (int i = 0; i < filter_size; i++) {
...
//                        printf("%u ", pA[row][col+layer*filter_size]);
//                    }
//                    printf("\n");
//                }
//                printf("\n");
//            }
//            start_time = clock();
            Mat_XNOR(xA, xB, xC, line);
//            stop_time = clock();
//            time = (int) (stop_time - start_time);
//            printf("XNOR time(cycles)=%d\n", time);
//    cudaThreadSynchronize();
//            printf("After XNOR function\n");
//            __syncthreads();
            output[fout_c][line] = result_device[line];
//            __syncthreads();
        }
//    }
//    pB = filter[0];
...
//    cudaMemcpyToSymbol(result_device, &result_host,sizeof(result_host), cudaMemcpyHostToDevice);
    ...
    dim3 NumberofBlocks(1,output_size);
